...
async function generateSitemap() {
  ...
  try {
    ...
    let listings = [];
    try {
      const response = await fetch(`${API_URL}/listings`, { timeout: 10000 });
      if (!response.ok) {
        throw new Error(`API responded with status: ${response.status}`);
      }
      const data = await response.json();
      listings = Array.isArray(data.rows)
        ? data.rows
        : Array.isArray(data)
          ? data
          : [];
      ...
      console.warn(
        `⚠️ Warning: Could not fetch listings for sitemap. Using static routes only. Error: ${fetchError.message}`,
      );
    }
