    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (data.ok) {
                ...
                setMessages(prev => [...prev, { role: "bot", text: "I apologize, but I am having trouble connecting to the concierge network. Please try again in a moment." }]);
            }
        } catch (err) {
            setMessages(prev => [...prev, { role: "bot", text: "I apologize, but I am currently offline. Please contact our human support team for immediate assistance." }]);
        } finally {
            setIsThinking(false);
        }
    };
                    ...
                    >
                        ...
                        <div ref={scrollRef} className="flex-1 p-6 overflow-y-auto space-y-6 bg-slate-50/50 no-scrollbar">
                            {messages.map((m, i) => (
                                ...
                                >
                                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${m.role === "user" ? "bg-slate-900 text-white" : "bg-white border border-slate-100 text-emerald-600 shadow-sm"}`}>
                                        {m.role === "user" ? <User size={14} /> : <Bot size={14} />}
                                    </div>
                                    <div className={`p-4 rounded-2xl text-[13px] leading-relaxed shadow-sm max-w-[80%] ${m.role === "user" ? "bg-slate-900 text-white rounded-tr-none" : "bg-white text-slate-600 border border-slate-50 rounded-tl-none italic font-medium"}`}>
                                        {m.text}
                                    </div>
                                </motion.div>
                            ))}
