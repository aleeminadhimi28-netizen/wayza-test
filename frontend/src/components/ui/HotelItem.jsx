...
export function HotelItem({ hotel, isSaved, onToggleWishlist }) {
  ...
  return (
    <div className="group relative">
      ...
      >
        <svg
          className={`w-5 h-5 ${isSaved ? 'fill-current' : 'fill-none stroke-current stroke-2'}`}
          viewBox="0 0 24 24"
        >
        ...
        <div className="flex flex-col gap-6">
          <div className="relative aspect-[4/5] overflow-hidden rounded-[32px] shadow-sm bg-slate-100">
            ...
            <div className="absolute top-6 left-6 flex flex-col gap-2">
              {isRare && (
                <span className="bg-white/95 backdrop-blur-md text-slate-950 text-[9px] font-bold px-4 py-2 rounded-full uppercase tracking-widest border border-white shadow-md">
                  Rare Find
                </span>
              )}
              ...
              {!isRare && hotel.price <= 8000 && (
                <span className="bg-emerald-500 text-white text-[9px] font-bold px-4 py-2 rounded-full uppercase tracking-widest shadow-lg shadow-emerald-500/10">
                  Trending
                </span>
              )}
            </div>
          ...
          <div className="px-1">
            <div className="flex justify-between items-start mb-1.5">
              <h3 className="text-xl font-bold text-slate-900 group-hover:text-emerald-600 transition-colors uppercase tracking-tight truncate pr-4">
                {hotel.name}
              </h3>
            ...
            <div className="flex justify-between items-center mb-4">
              <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest truncate pr-2">
                {hotel.location}
              </p>
              {hotel.wifiSpeed > 0 && (
