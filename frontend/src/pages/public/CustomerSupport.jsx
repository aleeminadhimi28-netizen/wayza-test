...
import {
    MessageSquarePlus, Send, ArrowLeft, Clock, CheckCircle, XCircle,
...
const STATUS_MAP = {
    ...
    closed: { label: "Closed", color: "bg-slate-100 text-slate-500 border-slate-200", icon: CheckCircle },
};
            ...
            <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white font-sans">
                ...
                <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-900 text-white py-16 overflow-hidden">
                    <div className="absolute inset-0 opacity-10" style={{
                        backgroundImage: 'radial-gradient(circle at 30% 40%, rgba(16,185,129,0.3) 0%, transparent 60%), radial-gradient(circle at 70% 80%, rgba(6,182,212,0.2) 0%, transparent 50%)'
                    }} />
                ...
                <div className="max-w-5xl mx-auto px-6 -mt-6">
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-8">
                        {[
                            { label: "Open Tickets", value: openCount, color: "text-emerald-600", bgColor: "bg-emerald-50 border-emerald-100", icon: Clock },
                            { label: "Resolved", value: closedCount, color: "text-slate-500", bgColor: "bg-slate-50 border-slate-100", icon: CheckCircle },
                            { label: "Total", value: tickets.length, color: "text-blue-600", bgColor: "bg-blue-50 border-blue-100", icon: Inbox },
                        ].map((kpi, i) => (
                            <motion.div
                                key={i}
                                ...
                                {loading ? (
                                    ...
                                    <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-sm">
                                        ...
                                        <p className="text-sm text-slate-500 mt-1 mb-6">
                                            {tickets.length === 0 ? "You haven't submitted any support tickets yet." : "No tickets match your filters."}
                                        </p>
                                        {tickets.length === 0 && (
                                            <button
                                                onClick={() => setView("new")}
                                                    ...
                                                    <div className="flex items-start justify-between gap-4">
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex items-center gap-2 mb-1">
                                                                <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase border ${st.color}`}>
                                                                    {st.label}
                                                                </span>
                                                                <span className="px-2 py-0.5 rounded-md bg-slate-100 text-slate-500 text-[10px] font-semibold">
                                                                    {CATEGORIES.find(c => c.value === ticket.category)?.icon} {ticket.category}
                                                                </span>
                                                                {hasAdminReply && (
                                                            ...
                                                            <p className="text-xs text-slate-500 mt-1 line-clamp-1">
                                                                {lastReply ? `${lastReply.from === "admin" ? "Support" : "You"}: ${lastReply.message}` : ticket.message}
                                                            </p>
                                                        </div>
                                                        <div className="flex flex-col items-end gap-2 shrink-0">
                                                            <span className="text-[10px] font-medium text-slate-400">
                                                                {new Date(ticket.updatedAt || ticket.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}
                                                            </span>
                            ...
                            <motion.div key="new" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                                ...
                                <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                                    <div className="p-6 border-b border-slate-100">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-500 mt-0.5">Fill in the details below and our team will get back to you</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <form onSubmit={submitTicket} className="p-6 space-y-5">
                                            ...
                                            <div className="grid grid-cols-3 gap-2">
                                                {CATEGORIES.map(cat => (
                                                    <button
                                                        ...
                                                        className={`p-3 rounded-xl border text-sm font-medium text-left transition-all ${category === cat.value
                                        ...
                                        {/* Subject */}
                                        <div className="space-y-2">
                                            <label className="text-xs font-semibold text-slate-700">Subject <span className="text-rose-400">*</span></label>
                                            <input
                                                required
                                                type="text"
                                        ...
                                        {/* Message */}
                                        <div className="space-y-2">
                                            <label className="text-xs font-semibold text-slate-700">Message <span className="text-rose-400">*</span></label>
                                            <textarea
                                                required
                                                rows={5}
                                        ...
                                        <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-4">
                                            ...
                                            <p className="text-xs text-blue-700 font-medium">
                                                Our support team typically responds within 24 hours. For urgent matters, please include your booking ID or account email.
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="flex-1">
                                                <div className="flex items-center gap-2 mb-2">
                                                    {(() => {
                                                        const st = STATUS_MAP[selected.status] || STATUS_MAP.open; return (
                                                            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase border ${st.color}`}>{st.label}</span>
                                                        );
                                                    })()}
                                                    <span className="px-2 py-0.5 rounded-md bg-slate-100 text-slate-500 text-[10px] font-semibold">
                                                        {CATEGORIES.find(c => c.value === selected.category)?.icon} {selected.category}
                                                    </span>
                                                </div>
                                                <h2 className="text-xl font-bold text-slate-900">{selected.subject}</h2>
                                                <p className="text-xs text-slate-400 mt-1">
                                                    Created {new Date(selected.createdAt).toLocaleString("en-IN", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div className="flex-1">
                                                <div className="flex items-center gap-2 mb-1">
                                                    ...
                                                    <span className="text-[10px] text-slate-400">
                                                        {new Date(selected.createdAt).toLocaleString("en-IN", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}
                                                    </span>
                                                </div>
                                        ...
                                        {selected.replies?.map((r, i) => {
                                            ...
                                            return (
                                                <div key={i} className={`flex gap-3 ${isAdmin ? "" : "flex-row"}`}>
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-xs font-bold ${isAdmin ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"}`}>
                                                        {isAdmin ? <Shield size={14} /> : (user?.email?.[0] || "U").toUpperCase()}
                                                    </div>
                                                    <div className="flex-1">
                                                        <div className="flex items-center gap-2 mb-1">
                                                        ...
                                                        <div className={`rounded-xl p-4 text-sm leading-relaxed ${isAdmin
                                    ...
                                    {selected.status !== "closed" ? (
                                        ...
                                        <div className="p-4 border-t border-slate-200 bg-slate-50 text-center">
                                            <p className="text-sm text-slate-500 font-medium flex items-center justify-center gap-2">
                                                <CheckCircle size={14} /> This ticket has been closed. Create a new ticket for further assistance.
                                            </p>
                                        </div>
                                    )}
                                </div>
