...
import { 
    ...
    Music, Camera, Palmtree, Wind, Waves, Coffee
} from "lucide-react";
...
export default function Experiences() {
    ...
    const fixImg = (img) => {
        if (!img) return "https://images.unsplash.com/photo-1682695797221-8164ff1fafc9?auto=format&fit=crop&w=1200&q=80";
    ...
    useEffect(() => {
        setLoading(true);
        api.getListings({ category: 'activity' })
            .then(data => {
            ...
            <div className="bg-white font-sans text-slate-900 selection:bg-amber-100 selection:text-amber-900 overflow-hidden">
                ...
                <section className="relative h-[90vh] min-h-[800px] flex items-center justify-center bg-slate-950 overflow-hidden">
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center gap-8">
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <button onClick={() => window.scrollTo({ top: 800, behavior: 'smooth' })} className="h-16 px-10 bg-white/10 backdrop-blur-md border border-white/20 text-white rounded-2xl font-bold uppercase text-[10px] tracking-widest hover:bg-white/20 transition-all">
                                        Browse Grid
                                    </button>
                                </div>
                            </div>
                    ...
                    <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-4 text-white/30">
                        <motion.div animate={{ y: [0, 10, 0] }} transition={{ repeat: Infinity, duration: 2 }} className="w-px h-16 bg-gradient-to-b from-amber-500 to-transparent" />
                    </div>
                </section>
                                ...
                                >
                                    <div className="relative aspect-[3/4] rounded-[48px] overflow-hidden bg-slate-100 shadow-xl transition-all duration-700 hover:shadow-2xl hover:shadow-amber-500/10 hover:-translate-y-2">
                                        ...
                                        <div className="absolute top-8 left-8 flex flex-col gap-2">
                                            <span className="bg-white/95 backdrop-blur-md text-slate-950 text-[9px] font-bold px-4 py-2 rounded-full uppercase tracking-widest shadow-lg">Verified Experience</span>
                                            {exp.price > 10000 && (
                                        ...
                                        <div className="absolute bottom-10 left-10 right-10 space-y-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[10px] font-bold text-white/70 uppercase tracking-[0.2em]">{exp.location || "Coastline"}</span>
                                            </div>
                                            <h3 className="text-3xl font-bold text-white uppercase leading-tight line-clamp-2">{exp.title}</h3>
                                            <div className="flex justify-between items-end pt-4 border-t border-white/10">
                                                <div>
                                                    <p className="text-[9px] font-bold text-white/40 uppercase tracking-widest mb-1">Starting From</p>
                ...
                <section className="py-48 bg-slate-950 relative overflow-hidden">
                    ...
                    <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row items-center gap-24">
                        <div className="flex-1 w-full relative">
                            ...
                            >
                                <img src="https://images.unsplash.com/photo-1544620347-c4fd4a3d5957?auto=format&fit=crop&w=1200&q=80" className="w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-[5s]" />
                        ...
                        <div className="flex-1 space-y-12">
                            ...
                            <button className="h-24 px-16 bg-white text-slate-950 hover:bg-amber-500 hover:text-white rounded-[32px] font-bold text-[12px] uppercase tracking-[0.4em] transition-all flex items-center gap-8 active:scale-95 group">
                                View Extreme Collection <ArrowRight size={20} className="group-hover:translate-x-3 transition-transform" />
                            </button>
                        </div>
                    </div>
                </section>
                        ...
                        >
                            ...
                            <div className="relative z-10 space-y-12">
                                ...
                                <div className="flex flex-col md:flex-row justify-center gap-6">
                                    <button onClick={() => navigate('/listings?category=activity')} className="h-24 px-16 bg-slate-950 text-white hover:bg-slate-800 rounded-3xl font-bold uppercase text-[12px] tracking-[0.4em] transition-all shadow-3xl active:scale-95">
                                        Start Your Discovery
                                    </button>
                                    <button onClick={() => navigate('/support')} className="h-24 px-16 bg-white/20 backdrop-blur-md border-2 border-slate-950/10 text-slate-950 hover:bg-white/30 rounded-3xl font-bold uppercase text-[12px] tracking-[0.4em] transition-all">
                                        Talk to an Expert
                                    </button>
                                </div>
                            </div>
                ...
                <footer className="bg-white py-48 px-6 md:px-12 border-t border-slate-100 relative">
                    <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-32">
                        <div className="md:col-span-6 space-y-16">
                            ...
                            <div className="flex gap-4">
                                {[Globe, Shield, Zap, Music].map((Icon, i) => (
                                    <div key={i} className="w-16 h-16 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-center text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition-all duration-500 cursor-pointer shadow-sm group">
                        ...
                        <div className="md:col-span-3 space-y-8">
                            <h3 className="text-slate-950 font-bold uppercase text-[11px] tracking-[0.6em] border-l-2 border-amber-500 pl-4">NAVIGATE</h3>
                            <ul className="space-y-6">
                                {['Native Secrets', 'Interactive Map', 'Our Stays', 'Partner Program'].map(l => (
                                    <li key={l}>
                                        <Link to="/listings" className="text-slate-400 font-bold text-lg hover:text-amber-600 transition-all uppercase tracking-tighter flex items-center gap-3">
                                            {l}
                                        </Link>
                                    </li>
                                ))}
                        ...
                        <div className="md:col-span-3 space-y-8">
                            <h3 className="text-slate-950 font-bold uppercase text-[11px] tracking-[0.6em] border-l-2 border-amber-500 pl-4">HELP</h3>
                            <ul className="space-y-6">
                                {['Support Center', 'Privacy', 'Terms', 'About'].map(l => (
                                    <li key={l}>
                                        <Link to="/support" className="text-slate-400 font-bold text-lg hover:text-amber-600 transition-all uppercase tracking-tighter flex items-center gap-3">
                                            {l}
                                        </Link>
                                    </li>
                                ))}
