...
import {
  Sparkles, MapPin, Clock, Users, Star, Check, ChevronRight,
  ChevronLeft, Waves, TreePine, Anchor, Mountain, Sun, Moon,
  Heart, Zap, Utensils, Car, Home, Camera, ArrowRight, Shield,
  Phone, Bike, Coffee, Music, Sunset
} from 'lucide-react';

// ── Data ───────────────────────────────────────────────────────────────────────

const VARKALA = { id: 'varkala', label: 'Varkala', sub: 'Clifftop & Beach', icon: Waves, img: 'https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=1600&q=80', base: 4500 };
...
const VIBES = [
  { id: 'relax', label: 'Relax & Unwind', icon: Moon, add: 0, color: 'from-blue-500/20 to-teal-500/20', border: 'border-teal-400' },
  { id: 'adventure', label: 'Adventure', icon: Zap, add: 1200, color: 'from-orange-500/20 to-red-500/20', border: 'border-orange-400' },
  { id: 'romance', label: 'Romance', icon: Heart, add: 2000, color: 'from-pink-500/20 to-rose-500/20', border: 'border-pink-400' },
  { id: 'culture', label: 'Cultural', icon: Coffee, add: 800, color: 'from-amber-500/20 to-yellow-500/20', border: 'border-amber-400' },
  { id: 'luxury', label: 'Luxury', icon: Star, add: 3500, color: 'from-purple-500/20 to-violet-500/20', border: 'border-purple-400' },
  { id: 'wellness', label: 'Wellness', icon: Sun, add: 1500, color: 'from-green-500/20 to-emerald-500/20', border: 'border-emerald-400' },
];

const ADDONS = [
  { id: 'spa', label: 'Ayurvedic Spa', icon: Sun, price: 2500, desc: '2-hr full-body treatment' },
  { id: 'cruise', label: 'Sunset Cruise', icon: Anchor, price: 1800, desc: 'Catamaran with canapes' },
  { id: 'cooking', label: 'Cooking Class', icon: Utensils, price: 1200, desc: 'Local chef, 3 dishes' },
  ...
  { id: 'music', label: 'Live Kathakali', icon: Music, price: 900, desc: 'Traditional performance' },
];
      ...
      {DURATIONS.map((d) => {
        ...
        return (
          ...
          >
            ...
            <div className={`text-2xl font-black mt-2 ${sel ? 'text-emerald-600' : 'text-slate-900'}`}>{d.label}</div>
      ...
      {VIBES.map((v) => {
        ...
        return (
          ...
          >
            ...
            {v.add > 0 && <div className="text-xs text-slate-500 mt-0.5">+₹{v.add.toLocaleString('en-IN')}</div>}
            {sel && <div className="mt-2 text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Selected ✓</div>}
          </button>
        );
      })}
    </div>
      ...
      {STAY_TYPES.map((s) => {
        ...
        return (
          ...
          >
            ...
            <div className={`font-black mt-2 ${sel ? 'text-emerald-700' : 'text-slate-900'}`}>{s.label}</div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {ADDONS.map((a) => {
        ...
        return (
          ...
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${sel ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-400'}`}>
              <Icon size={16} />
            </div>
            ...
            <div className={`text-sm font-black flex-shrink-0 ${sel ? 'text-emerald-600' : 'text-slate-400'}`}>+₹{a.price.toLocaleString('en-IN')}</div>
          </button>
        );
      })}
    </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setGuests(Math.max(1, guests - 1))} className="w-11 h-11 rounded-full border-2 border-slate-200 flex items-center justify-center text-xl font-black text-slate-700 hover:border-emerald-400 transition-colors">−</button>
          ...
          <button onClick={() => setGuests(Math.min(12, guests + 1))} className="w-11 h-11 rounded-full border-2 border-slate-200 flex items-center justify-center text-xl font-black text-slate-700 hover:border-emerald-400 transition-colors">+</button>
        </div>
      </div>
      <div>
          ...
          {TRANSPORT.map((t) => {
            ...
            return (
              ...
              >
                ...
                <div className={`text-xs mt-0.5 font-semibold ${sel ? 'text-emerald-600' : 'text-slate-400'}`}>{t.price === 0 ? 'Free' : `+₹${t.price.toLocaleString('en-IN')}`}</div>
              </button>
            );
          })}
        </div>
...
export default function TourPackager() {
  ...
  const stepContent = [
    { title: 'How long is your trip?', sub: 'Select your preferred duration', component: <StepDuration value={duration} onChange={setDuration} /> },
    { title: 'What\'s your travel vibe?', sub: 'We\'ll tailor every detail to match', component: <StepVibe value={vibe} onChange={setVibe} /> },
    { title: 'Choose your stay type', sub: 'From cozy guesthouses to clifftop suites', component: <StepStay value={stay} onChange={setStay} /> },
    { title: 'Add experiences', sub: 'Optional extras to make it unforgettable', component: <StepAddons value={addons} onChange={setAddons} /> },
    { title: 'Finalize your trip', sub: 'Travellers and transport preferences', component: <StepGuests guests={guests} setGuests={setGuests} transport={transport} setTransport={setTransport} /> },
  ];
    ...
    <WayzzaLayout noPadding>
      ...
      <header className="relative bg-slate-950 text-white overflow-hidden min-h-[380px] flex items-end">
        ...
        <div className="relative w-full max-w-5xl mx-auto px-6 py-16">
          ...
          >
            Build Your Varkala<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-300">Dream Package.</span>
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Choose your destination, vibe, stay, and experiences. Get an instant price — then book it all in one click.
          </motion.p>
        </div>
      </header>
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-6 items-start">
              ...
              {STEPS.map((s, i) => (
                <button
                  ...
                  className={`flex-shrink-0 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                    step === i ? 'bg-slate-950 text-white' : i < step ? 'text-emerald-600 bg-emerald-50' : 'text-slate-400 hover:text-slate-600'
                  }`}
                >
                  {i < step ? '✓ ' : `${i + 1}. `}{s}
                </button>
          ...
          <div className="space-y-4 lg:sticky lg:top-24">
            ...
            <div className="bg-slate-950 text-white rounded-[24px] p-6">
              <div className="text-xs uppercase tracking-widest text-slate-400 font-bold mb-1">Your Package</div>
              <div className="text-4xl font-black text-emerald-400 mt-2">
                ₹{totalPrice.toLocaleString('en-IN')}
              </div>
              <div className="text-slate-400 text-xs mt-1">total for {guests} {guests === 1 ? 'person' : 'people'}</div>
            ...
            <div className="bg-white rounded-[20px] border border-slate-100 p-4 space-y-3">
              {[
                ...
                <div key={label} className="flex items-center gap-3 text-sm text-slate-600 font-medium">
