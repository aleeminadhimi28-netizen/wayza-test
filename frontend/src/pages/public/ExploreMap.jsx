            ...
            <div className="h-screen flex flex-col pt-20 overflow-hidden font-sans">
                    ...
                    <div className="flex gap-3">
                        <div className="hidden sm:flex items-center gap-4 bg-slate-50 px-4 py-2 rounded-xl border border-slate-200">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[11px] font-bold text-slate-600 uppercase tracking-widest">{listings.length} Properties found</span>
                            </div>
                        </div>
                        <Link to="/listings" className="h-10 px-6 bg-slate-900 text-white rounded-xl flex items-center gap-2 font-bold text-xs uppercase tracking-widest hover:bg-emerald-600 transition-all active:scale-95 shadow-lg shadow-slate-900/10">
                ...
                <div className="flex-1 relative">
                    {loading ? (
                        <div className="absolute inset-0 z-[1001] bg-white/80 backdrop-blur-sm flex items-center justify-center">
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <p className="text-sm font-bold text-slate-700 tracking-tight uppercase tracking-widest">Initializing Map...</p>
                            </div>
                        </div>
                    ) : error ? (
                        <div className="absolute inset-0 z-[1001] bg-white flex items-center justify-center">
                            <div className="text-center p-8">
                                ...
                                <button onClick={() => window.location.reload()} className="h-11 px-8 bg-slate-900 text-white rounded-xl font-bold text-sm">Retry</button>
                            </div>
                        </div>
                    ) : null}
                                ...
                                <Popup className="wayza-custom-popup">
                                    <div className="w-64 overflow-hidden rounded-xl">
                                        <div className="relative h-32 bg-slate-100">
                                            {item.image ? (
                                                <img src={api.fixImg(item.image)} className="w-full h-full object-cover" alt={item.title} />
                                            ) : (
                                            ...
                                            <div className="absolute top-3 left-3 bg-white/95 backdrop-blur px-2.5 py-1 rounded-lg shadow-sm border border-black/5">
                                                <span className="text-[10px] font-black tracking-tight text-slate-900 uppercase">₹{item.price.toLocaleString()}</span>
                                            </div>
                                        </div>
                                        <div className="p-4 bg-white">
                                            <div className="flex items-center gap-1.5 mb-1">
                                                <Star size={10} className="text-amber-400 fill-amber-400" />
                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">New Property</span>
                                            </div>
                                            <h3 className="text-sm font-bold text-slate-900 mb-1 truncate">{item.title}</h3>
                                            ...
                                            <Link to={`/listings/${item._id}`} className="block w-full text-center py-2.5 bg-emerald-600 text-white rounded-lg font-bold text-[10px] uppercase tracking-widest hover:bg-slate-900 transition-all shadow-lg shadow-emerald-500/10">
                                                Book Now
                                            </Link>
                                        </div>
                                    </div>
                    ...
                    <div className="absolute right-6 top-6 flex flex-col gap-2 z-[1000]">
                        <button onClick={() => setMapZoom(prev => Math.min(prev + 1, 18))} className="w-12 h-12 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-700 shadow-xl hover:bg-slate-50 transition-all font-bold text-lg">+</button>
                        <button onClick={() => setMapZoom(prev => Math.max(prev - 1, 3))} className="w-12 h-12 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-700 shadow-xl hover:bg-slate-50 transition-all font-bold text-lg">−</button>
                    </div>
                    ...
                    <div className="absolute left-6 bottom-6 right-6 lg:right-auto lg:w-96 z-[1000]">
                        <AnimatePresence>
                            {activeItem ? (
                                ...
                                >
                                    <button onClick={() => setActiveItem(null)} className="absolute -top-3 -right-3 w-8 h-8 bg-slate-900 text-white rounded-full flex items-center justify-center shadow-lg border-2 border-white hover:bg-rose-500 transition-all">
                                    ...
                                    <div className="flex gap-4">
                                        <div className="w-24 h-24 rounded-2xl bg-slate-100 overflow-hidden shrink-0 border border-black/5">
                                            {activeItem.image ? (
                                                ...
                                                <div className="w-full h-full flex items-center justify-center text-slate-300 bg-slate-50"><Home size={24} /></div>
                                            )}
                                        </div>
                                        <div className="flex-1 min-w-0 flex flex-col">
                                            <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mb-1">{activeItem.category || "Property"}</p>
                                            <h3 className="text-lg font-bold text-slate-900 truncate mb-1">{activeItem.title}</h3>
                                            ...
                                            <div className="flex items-center justify-between mt-auto">
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Price Starts</span>
                                                    <span className="text-sm font-black text-slate-900">₹{activeItem.price.toLocaleString()}<span className="text-[10px] font-bold text-slate-400">/night</span></span>
                                                </div>
                                                <Link to={`/listings/${activeItem._id}`} className="h-9 px-5 bg-slate-900 text-white rounded-xl flex items-center gap-1.5 font-bold text-[10px] uppercase tracking-widest hover:bg-emerald-600 transition-all active:scale-95 shadow-lg shadow-slate-950/20">
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        <p className="text-sm font-bold tracking-tight">Click on a marker to see details</p>
                                        <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">Explore available properties on the map</p>
                                    </div>
                                </motion.div>
                            )}
                        </AnimatePresence>
