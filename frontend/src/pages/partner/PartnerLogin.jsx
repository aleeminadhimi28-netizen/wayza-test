        ...
        <div className="min-h-screen bg-white font-sans flex flex-col md:flex-row overflow-hidden selection:bg-emerald-100 selection:text-emerald-900">
            ...
            <div className="hidden md:flex md:w-[45%] bg-slate-950 text-white p-20 flex-col justify-between relative overflow-hidden border-r border-white/5">
                ...
                <div className="relative z-10 flex items-center gap-12">
                    <div className="flex -space-x-4">
                        {[1, 2, 3, 4].map(i => (
                            <div key={i} className="w-12 h-12 rounded-full border-2 border-slate-900 bg-slate-800 overflow-hidden shadow-xl">
                                <img src={`https://i.pravatar.cc/150?u=${i + 10}`} alt="Partner" className="w-full h-full object-cover grayscale opacity-50" />
                            </div>
                        ))}
                    ...
                    <span className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em]">Trusted by elite hosts worldwide</span>
                </div>
            </div>
                    ...
                    >
                        <header className="space-y-8">
                            <div className="flex items-center gap-4">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-[0.5em] text-emerald-600">Access Key Req.</span>
                            </div>
                        ...
                        <form onSubmit={login} className="space-y-8">
                            <div className="space-y-3 group/field">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] ml-2 group-focus-within/field:text-emerald-600 transition-colors">Credential Email</label>
                                <div className="relative">
                                    <div className="absolute left-6 top-1/2 -translate-y-1/2 w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center group-focus-within/field:bg-emerald-50 transition-colors">
                                        <Mail className="text-slate-300 group-focus-within/field:text-emerald-500 transition-colors" size={20} />
                                    </div>
                                    <input
                            ...
                            <div className="space-y-3 group/field">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] ml-2 group-focus-within/field:text-emerald-600 transition-colors">Access Password</label>
                                <div className="relative">
                                    <div className="absolute left-6 top-1/2 -translate-y-1/2 w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center group-focus-within/field:bg-emerald-50 transition-colors">
                                        <Lock className="text-slate-300 group-focus-within/field:text-emerald-500 transition-colors" size={20} />
                                    </div>
                                    <input
                        ...
                        <div className="pt-12 flex flex-col items-center gap-8 text-center border-t border-slate-100">
                            <div className="space-y-3">
                                ...
                                <Link to="/partner-register" className="inline-flex items-center gap-4 text-emerald-600 font-black text-[10px] uppercase tracking-[0.5em] hover:text-emerald-700 transition-all group">
                                    Establish Residency <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
                                </Link>
                            </div>

                            <Link to="/login" className="inline-flex items-center gap-4 text-slate-300 font-bold text-[9px] uppercase tracking-[0.4em] hover:text-slate-950 transition-colors pt-6 group">
                                <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> Back to Guest Access
                            </Link>
                        </div>
                    </motion.div>
                </div>
