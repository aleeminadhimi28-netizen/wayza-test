...
import {
  Home, Plus, Trash2, Edit3, MapPin, ShieldCheck, Clock,
  Activity, Building, ChevronRight, AlertCircle
} from "lucide-react";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          <AnimatePresence>
            {listings.map((listing, i) => {
              const lowestPrice = listing.variants?.length > 0
                ...
                >
                  ...
                  >
                    ...
                    <div className="absolute top-3 left-3 flex flex-col gap-2">
                      ...
                      <div className={`backdrop-blur-sm px-3 py-1 rounded-lg text-white font-bold text-xs flex items-center gap-1.5 ${listing.approved ? "bg-emerald-500/90" : "bg-amber-500/90"}`}>
                    ...
                    <div className="flex items-center justify-between border-t border-slate-100 pt-4">
                      <div>
                        ...
                        <p className="text-xl font-bold text-slate-900">
                          {lowestPrice > 0 ? `₹${lowestPrice.toLocaleString()}` : <span className="text-slate-400 text-sm font-semibold">No price set</span>}
                        </p>
                        {listing.variants?.length > 0 && (
                          <p className="text-xs text-emerald-600 font-medium mt-0.5">{listing.variants.length} room type{listing.variants.length > 1 ? "s" : ""}</p>
                        )}
                      </div>
