...
import {
  ...
  XCircle, ChevronDown, Scan, Camera, Edit3, Navigation, ArrowRight, Shield, Zap
} from "lucide-react";
...
const STATUS_CONFIG = {
  ...
  arrived: { label: "In-Stay", color: "bg-blue-50 text-blue-700 border-blue-100", dot: "bg-blue-500", icon: Navigation },
  departed: { label: "Completed", color: "bg-slate-50 text-slate-700 border-slate-100", dot: "bg-slate-500", icon: Shield },
...
export default function PartnerBookings() {
  ...
  const totalRevenue = visible.filter(b => b.status === "paid" || b.status === "arrived" || b.status === "departed").reduce((s, b) => s + (b.totalPrice || 0), 0);
        ...
        {scannerActive && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
            ...
            >
              ...
              <button 
                onClick={() => { window.stopScanner?.(); setScannerActive(false); }} 
              ...
              <div className="space-y-6">
                <div className="text-center space-y-1">
                  <p className="text-[10px] font-black uppercase tracking-[0.4em] text-emerald-600">Verification Hub</p>
                ...
                {!scanResult ? (
                  <div className="space-y-6">
                    ...
                    <div className="relative aspect-square bg-slate-900 rounded-[32px] overflow-hidden shadow-inner border-4 border-slate-100">
                      ...
                      {scanning && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-white pointer-events-none">
                          ...
                          <p className="text-[10px] font-bold uppercase tracking-widest mt-6 opacity-60">Scanning for passport...</p>
                        </div>
                      )}
                      {!scanning && !manualCode && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-white bg-slate-900/80 p-8 text-center gap-4">
                           ...
                           <button onClick={startScanner} className="px-6 py-2 bg-emerald-600 rounded-xl text-xs font-bold uppercase">Enable Camera</button>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="relative">
                      ...
                      <div className="relative flex justify-center text-[10px]"><span className="px-4 bg-white text-slate-400 font-black uppercase tracking-widest">Or Manual</span></div>
                    </div>
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                    <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-[32px] text-center space-y-4">
                       ...
                       <div>
                         <p className="text-xs font-black text-emerald-800 uppercase tracking-widest">Identity Confirmed</p>
                         <h4 className="text-xl font-bold text-slate-900 mt-1 uppercase">{scanResult.guestEmail.split('@')[0]}</h4>
                       </div>
                    </div>
                    ...
                    <div className="space-y-3">
                       <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                          ...
                          <span className="text-xs font-bold text-slate-900 uppercase">Check-In Verification</span>
                       </div>
                       <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Property</span>
                          <span className="text-xs font-bold text-slate-900 uppercase truncate max-w-[120px]">{scanResult.title}</span>
                       </div>
                    </div>
