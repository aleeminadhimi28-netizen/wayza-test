...
import {
    Wallet, CheckCircle, Clock, ArrowDownCircle,
    Building2, CreditCard, ShieldCheck, Send, AlertCircle,
    TrendingUp, RefreshCcw, Landmark, Zap, Info
} from "lucide-react";
...
export default function PartnerWallet() {
    ...
    // Bank form state
    const [form, setForm] = useState({ accountName: "", accountNumber: "", ifscCode: "", bankName: "", upiId: "" });
    ...
    async function handleWithdraw(e) {
        ...
        try {
            ...
            if (d.ok) {
                setWithdrawMsg({ type: "success", text: `₹${Number(withdrawAmount).toLocaleString()} withdrawal request submitted!` });
                setWithdrawAmount("");
                // Refresh requests
    ...
    const balanceCards = [
        { label: "Available to Withdraw", value: available, icon: Wallet, bg: "from-emerald-500 to-teal-600", desc: "Ready to request" },
        { label: "Pending Settlement", value: pending, icon: Clock, bg: "from-amber-500 to-orange-500", desc: "Awaiting check-in" },
        { label: "Already Paid", value: alreadyPaid, icon: CheckCircle, bg: "from-blue-500 to-indigo-600", desc: "Transferred to bank" },
        { label: "Total Earned", value: total, icon: TrendingUp, bg: "from-slate-700 to-slate-900", desc: "All time gross" },
    ];

    return (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 font-sans pb-16">
            ...
            <div>
                ...
                <p className="text-sm text-slate-500 mt-1">Manage your bank details and request withdrawals.</p>
            </div>
                ...
                {balanceCards.map((c, i) => (
                    ...
                    >
                        ...
                        <div className="relative z-10">
                            ...
                            <p className="text-white/50 text-[10px] font-medium uppercase tracking-widest mt-1">{c.desc}</p>
                        </div>
                    </motion.div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
                    ...
                    <div className="flex items-center justify-between bg-emerald-50 border border-emerald-100 rounded-xl px-5 py-4 mb-6">
                        <div>
                            <p className="text-xs font-semibold text-emerald-600 uppercase tracking-wide">Available Balance</p>
                            <p className="text-2xl font-bold text-emerald-700 mt-0.5">₹{Math.round(available).toLocaleString()}</p>
                        </div>
                    ...
                    <form onSubmit={handleWithdraw} className="space-y-4">
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase tracking-wide block mb-1.5">Withdrawal Amount (₹)</label>
                        ...
                        <AnimatePresence>
                            {withdrawMsg && (
                                ...
                                >
                                    {withdrawMsg.type === "success" ? <CheckCircle size={14} /> : <AlertCircle size={14} />}
                                    {withdrawMsg.text}
                                </motion.div>
                            )}
                        </AnimatePresence>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
                    ...
                    <form onSubmit={handleSaveBank} className="space-y-4">
                        {[
                            { label: "Account Holder Name", key: "accountName", placeholder: "e.g. John Doe", icon: CreditCard },
                            { label: "Account Number", key: "accountNumber", placeholder: "e.g. 123456789012", icon: Building2 },
                            { label: "IFSC Code", key: "ifscCode", placeholder: "e.g. SBIN0001234", icon: Info },
                            { label: "Bank Name", key: "bankName", placeholder: "e.g. State Bank of India", icon: Landmark },
                        ].map(f => (
                            <div key={f.key}>
                                <label className="text-xs font-bold text-slate-500 uppercase tracking-wide block mb-1.5">{f.label}</label>
                                <div className="relative">
                                    <f.icon size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
                                    <input
                                        type="text"
                                        value={form[f.key]}
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase tracking-wide block mb-1.5">UPI ID <span className="text-slate-400 normal-case font-normal">(optional)</span></label>
                            <input
                                type="text"
                                value={form.upiId}
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-100 bg-slate-50/50">
                                ...
                                <th className="px-6 py-4 text-right text-xs font-bold text-slate-400 uppercase tracking-wide">Reference</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                            {requests.map((r, i) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 font-bold text-sm text-slate-900">₹{Number(r.amount).toLocaleString()}</td>
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border ${r.status === "completed" || r.status === "paid"
                                                ? "bg-emerald-50 text-emerald-700 border-emerald-100"
                                                : r.status === "rejected"
                                                    ? "bg-rose-50 text-rose-700 border-rose-100"
                                                    : "bg-amber-50 text-amber-700 border-amber-100"
                                            }`}>
                                            {r.status === "completed" || r.status === "paid" ? <CheckCircle size={10} /> : <Clock size={10} />}
                                            {r.status}
                                        </span>
                                    </td>
                            ...
                            {requests.length === 0 && (
                                <tr>
                                    <td colSpan={4} className="py-20 text-center">
                                        ...
                                        <p className="text-sm font-semibold text-slate-900 mb-1">No withdrawal requests yet</p>
                                        <p className="text-xs text-slate-500">Your payout history will appear here once you submit a request.</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
