...
import {
  LayoutDashboard, Users, Home, Activity, Search, Filter, Plus,
  DollarSign, Star, Calendar, CheckCircle, Clock, ArrowUpRight,
  MoreVertical, ChevronRight, Bell, Sparkles, ShieldCheck, TrendingUp, Navigation, CalendarCheck
} from "lucide-react";
...
export default function PartnerDashboard() {
  ...
  const kpis = [
    { label: "Available Balance", value: "₹" + (earnings?.availableBalance || 0).toLocaleString(), icon: DollarSign, color: "text-emerald-600", bg: "bg-emerald-50", trend: "Ready to transfer", up: true },
    { label: "Pending Settlement", value: "₹" + (earnings?.pendingBalance || 0).toLocaleString(), icon: Clock, color: "text-amber-600", bg: "bg-amber-50", trend: "Upcoming stays", up: true },
    { label: "Already Paid", value: "₹" + (earnings?.alreadyPaid || 0).toLocaleString(), icon: CheckCircle, bg: "bg-blue-50", color: "text-blue-600", trend: "Settled", up: true },
    { label: "Total Bookings", value: total, icon: Users, color: "text-slate-900", bg: "bg-slate-50", trend: "All time", up: true },
  ];
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 font-sans selection:bg-emerald-100 selection:text-emerald-900 pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-slate-500 text-sm">Here's what's happening with your properties today.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map((c, i) => (
          <motion.div
            key={c.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
          ...
          >
            <div className="flex justify-between items-start">
              ...
              <div className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-bold ${c.up ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
                {c.up ? <ArrowUpRight size={12} strokeWidth={3} /> : <Clock size={12} strokeWidth={3} />}
                {c.trend}
              </div>
            </div>
            <div>
          ...
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={monthly}>
                ...
                <YAxis tick={{ fontSize: 11, fill: "#64748b" }} axisLine={false} tickLine={false} dx={-10} tickFormatter={(val) => `₹${val}`} />
                <Tooltip
                  contentStyle={{ background: '#0f172a', border: 'none', borderRadius: '12px', color: '#fff', fontSize: '12px', padding: '12px' }}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 rounded-3xl p-8 text-white relative overflow-hidden shadow-lg group">
            ...
            <div className="relative z-10 space-y-4">
              <div className="hidden sm:block">
                ...
                <p className="text-slate-400 text-xs leading-relaxed">Join the Premium Partner Program to boost your search rankings by up to 45% and access exclusive insights.</p>
              </div>
              <button className="w-full h-10 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-semibold text-xs transition-colors">Learn More</button>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: "Properties", icon: Home, color: "bg-blue-50 text-blue-600", path: "/partner/properties" },
                { label: "Earnings", icon: DollarSign, color: "bg-emerald-50 text-emerald-600", path: "/partner/earnings" },
                { label: "Reviews", icon: Star, color: "bg-amber-50 text-amber-600", path: "/partner/reviews" },
                { label: "Calendar", icon: Calendar, color: "bg-purple-50 text-purple-600", path: "/partner/calendar" }
              ].map(a => (
                <button key={a.label} onClick={() => navigate(a.path)} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 hover:bg-white hover:shadow-md border border-transparent hover:border-slate-200 transition-all text-left">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${a.color} shadow-sm`}>
                    <a.icon size={16} />
                  </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {visible.slice(0, 8).map((b, i) => (
                <motion.tr
                  key={b._id || i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }}
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-slate-900 text-sm truncate max-w-[200px]">{b.title || "Untitled Property"}</p>
                        <p className="text-xs text-slate-500">{b.guestEmail?.split('@')[0]}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-600">
                    <div className="whitespace-nowrap">{b.checkIn} <span className="text-slate-300 mx-1">→</span> {b.checkOut}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1.5">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold ${b.status === "paid" ? "bg-emerald-100 text-emerald-700" :
                        b.status === "cancelled" ? "bg-rose-100 text-rose-700" :
                          "bg-amber-100 text-amber-700"
                        }`}
                      >
                        <div className={`w-1.5 h-1.5 rounded-full ${b.status === 'paid' ? 'bg-emerald-500' : b.status === 'cancelled' ? 'bg-rose-500' : 'bg-amber-500'}`} />
                        {b.status === 'paid' ? 'Confirmed' : b.status === 'cancelled' ? 'Cancelled' : 'Pending'}
                      </span>
                      {b.status === "paid" && (
                        <span className={`text-[10px] font-bold uppercase ${b.payoutStatus === "paid_out" ? "text-blue-500" : "text-amber-500"}`}>
                  ...
                  <td className="px-6 py-4 text-right">
                    <p className="font-bold text-slate-900">₹{Math.round(b.netEarnings || (b.totalPrice * 0.9)).toLocaleString()}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Partner Share</p>
                  </td>
                </motion.tr>
              ))}
